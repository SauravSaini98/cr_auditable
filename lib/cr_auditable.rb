...
ActiveSupport.on_load(:active_record) do
  class ActiveRecord::Base
    def self.save_auditable_records(association_name, scope = nil, **options, &extension)
      if included_modules.include?(CrAuditable)
        puts "[WARN] #{self.name} is calling save_auditable_records more than once!"
        return
      end
...
module CrAuditableControllerExtension
  extend ActiveSupport::Concern

  private

  def set_cr_auditable_user!(model)
...
ActiveSupport.on_load(:action_controller) do
  include CrAuditableControllerExtension
end

require 'cr_auditable/rspec' if defined? RSpec
